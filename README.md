...
* [`src/clj/whiteboard/`](src/clj/whiteboard/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/whiteboard/`](src/cljs/whiteboard/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/whiteboard/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/whiteboard/`](test/cljs/whiteboard/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `whiteboard` project root directory
...
in the [`src/clj/whiteboard/`](src/clj/whiteboard/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/whiteboard/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns whiteboard.example
  (:require [whiteboard.config :as config])
